...
from fileutils import read_obj, write_obj, get_next_file_number, get_paths
from tracepoint import TracePath

@click.command()
@click.argument("longpath-dir")
@click.option("-p", "--padding", type=click.INT, required=False, default=0,
    ...
    """)
def generate_subpaths(longpath_dir, padding):
    if not os.path.exists(longpath_dir):
        print("Invalid long path dir provided.")
        sys.exit(1)

    paths = get_paths(longpath_dir)
    for longpath in paths:
        ...
        for subpath_digit_length in range(1, num_classes):
            ...
            for left_digit_checkpoint_index in range(0, len(checkpoint_indices) - (subpath_digit_length * 2) + 1, 2):
                ...
                output_file_folder = "{}/{}_digits/{}".format(base_folder_name, len(included_class_names), "_".join(included_class_names))
